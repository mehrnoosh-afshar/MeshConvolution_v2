...
import numpy as np
from torch_geometric.nn import SplineConv, ChebConv, GMMConv, GATConv, FeaStConv, BatchNorm, avg_pool_neighbor_x,GATv2Conv, ARMAConv,\
                                TransformerConv
...
class GVAE(nn.Module):
    def __init__(self, param, test_mode = False):
        ...
        self.layer_num = len(self.type_layer_lst)
            ...
            if layer_type == "c":
                ...
                if (conv_method == "Cheb"):
                    ...
                    if ((L != ((self.layer_num / 2) - 2)) and (L != (self.layer_num - 1))):  # not middle or last layer
                        ...
                        conv_layer = edge_index_lst, GATv2Conv(in_channel, out_channel, heads=2, concat=True)
                    ...
                    for p in conv_layer[-1].parameters():
                        ...
                        ii += 1
                    print("GAT OUTCHANNEL" , out_channel)
                    ...
                    conv_layer = edge_index_lst, ARMAConv(in_channel, out_channel, num_stacks = 4, num_layers = 2, shared_weights= True)
                    ii = 0
                    for p in conv_layer[-1].parameters():
                        p.data = p.data.cuda()
                   ...
                   if ((L != ((self.layer_num / 2) - 2)) and (L != (self.layer_num - 1))):  # not middle or last layer
                        conv_layer = edge_index_lst, TransformerConv(in_channel,out_channel,heads =2)
                        out_channel = out_channel * 2
                   else:
                        conv_layer = edge_index_lst, TransformerConv(in_channel,out_channel,heads =2, concat=False)
                ...
                zeros_batch_outpn_outchannel = torch.zeros((self.batch, out_point_num, out_channel)).cuda()
    ...
    def forward_one_layer_perBatch(self,x_batch,layer_info,is_final_layer = False):
        ...
        # print("here" , x_batch.shape)
        
        in_channel, out_channel, in_pn, out_pn, weight_num,  max_neighbor_num, neighbor_num_lst,neighbor_id_lstlst, layer_property,\
        ...
        if layer_type == "c":
            ####compute output of convolution layer####
            out_x_conv = torch.FloatTensor([]).cuda()
            if ((conv_method == "GAT") or (conv_method == "Cheb") or (conv_method == "FeaST")or (conv_method == "ARMA")or (conv_method == "Trans")):
                ...
                for b in range(batch):
                   ...
                   out_x_conv = torch.cat((out_x_conv, out_x_one.unsqueeze(0)), 0)
            elif ((conv_method == "GMM")):
                edge_index_lst, pseudo_coordinates, conv = layer_property
                for b in range(batch):
                    out_x_one = conv(x_batch[b], edge_index_lst, pseudo_coordinates)
                    out_x_conv = torch.cat((out_x_conv, out_x_one.unsqueeze(0)), 0)
            elif (conv_method == "vw"):
                ...
                out_x_conv = out_x_conv + bias

            elif (conv_method == "full"):

               (weights, bias, raw_w_weights) = layer_property #weight_num*(out_channel*in_channel)   out_point_num* max_neighbor_num* weight_num
            ...
            if (is_final_layer == False):
                out_x_conv = self.activation(out_x_conv)
